...
import { Save, RefreshCw, Filter, Search, Calendar, Layers, Package, ShoppingCart, Truck, AlertTriangle } from 'lucide-react';
...
function App() {
  ...
  const [appliedFilters, setAppliedFilters] = useState({
    spo: '', product: '', pStartFrom: '', pStartTo: '', exFactFrom: '', exFactTo: '', materialStatus: '', materialType: ''
  });
  ...
  const [appliedSchedFilters, setAppliedSchedFilters] = useState({
    spo: '', startFrom: '', startTo: '', exFrom: '', exTo: ''
  });
  ...
  const loadFromFirebase = async (isInitialLoad = false) => {
    if (!isInitialLoad) {
      Swal.fire({ title: 'Sinkronisasi Server...', text: 'Menarik data terbaru', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
    }
    try {
      const querySnapshot = await getDocs(collection(db, "shipment_monitoring"));
      const firebaseData = [];
  ...
  const handleProcessCSV = async () => {
    if (!outsFile || !poFile || !bookFile) {
      return Swal.fire('Peringatan', 'Harap upload file Outs, PO, dan Book sekaligus!', 'warning');
    }
    setIsProcessing(true);
    Swal.fire({ title: 'Menyinkronkan Data...', text: 'Memetakan alokasi silang logistik pabrik...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
    await new Promise(resolve => setTimeout(resolve, 150));
    try {
      const gasUrl = "https://script.google.com/macros/s/AKfycbwf2SLLQRtjUNLSaHTqUcdWlULexrYa7Je3PVgILaCgsdDHIttGFhNYd7dsQ-awC15l/exec"; // Ganti URL GAS Anda di sini
      ...
      const processed = processDashboardData(outsData, poData, monitoringAPIResult, bookData);
      setData(processed);
      Swal.fire({ icon: 'success', title: 'Berhasil', text: 'Database dialokasikan!', timer: 1500, showConfirmButton: false });
    } catch (error) {
      Swal.fire('Error Handling CSV', error.message || 'Gagal memproses.', 'error');
    } finally { setIsProcessing(false); }
  };

  const saveToFirebase = async () => {
    if (data.length === 0) return Swal.fire('Kosong', 'Tidak ada data!', 'warning');
    setIsProcessing(true);
    try {
  ...
  const handleApplyFilter = () => {
    Swal.fire({
      title: 'Memfilter Data...', text: 'Menyinkronkan tabel material', allowOutsideClick: false, timer: 400, showConfirmButton: false,
      didOpen: () => Swal.showLoading(),
      willClose: () => {
        setAppliedFilters({ spo: filterSPO, product: filterProduct, pStartFrom, pStartTo, exFactFrom, exFactTo, materialStatus: filterMatStatus, materialType: filterMatType });
      }
    });
  };
  ...
  const handleApplySchedFilter = () => {
    Swal.fire({
      title: 'Menyaring Jadwal...', text: 'Mengakumulasi kebutuhan purchasing', allowOutsideClick: false, timer: 500, showConfirmButton: false,
      didOpen: () => Swal.showLoading(),
      willClose: () => {
        setAppliedFilters({ spo: '', product: '', pStartFrom: '', pStartTo: '', exFactFrom: '', exFactTo: '', materialStatus: '', materialType: '' }); // Clear page 1 state filter context
        setAppliedSchedFilters({ spo: schedFilterSPO, startFrom: schedStartFrom, startTo: schedStartTo, exFrom: schedExFrom, exTo: schedExTo });
      }
    });
  };

  const clearSchedFilters = () => {
    setSchedFilterSPO(''); setSchedStartFrom(''); setSchedStartTo(''); setSchedExFrom(''); setSchedExTo('');
    setAppliedSchedFilters({ spo: '', startFrom: '', startTo: '', exFrom: '', exTo: '' });
  };

  // --- LOGIKA FILTER TABEL HALAMAN 1 ---
  const filteredDataTab1 = data.map(item => {
    const filteredMaterials = item.materials?.filter(mat => {
      if (appliedFilters.materialType && mat.group !== appliedFilters.materialType) return false;
      if (appliedFilters.materialStatus) {
        const status = appliedFilters.materialStatus;
        if (status === 'sudah_book' && mat.statusType !== 'booked') return false;
        if (status === 'free_stock' && !mat.statusType.includes('freestock')) return false;
        if (status === 'belum_po' && mat.statusType !== 'not_ordered' && mat.statusType !== 'freestock_kurang_belum_po') return false;
      }
      return true;
    }) || [];
    return { ...item, materials: filteredMaterials, groups: Array.from(new Set(filteredMaterials.map(m => m.group))) };
  }).filter(item => {
    const matchSPO = item.spo.toLowerCase().includes(appliedFilters.spo.toLowerCase());
    const matchProduct = item.product.toLowerCase().includes(appliedFilters.product.toLowerCase());
    let matchDates = true;
    if (appliedFilters.pStartFrom && new Date(item.pStart) < new Date(appliedFilters.pStartFrom)) matchDates = false;
    if (appliedFilters.pStartTo && new Date(item.pStart) > new Date(appliedFilters.pStartTo)) matchDates = false;
    if (appliedFilters.exFactFrom && item.exFact !== 'N/A' && new Date(item.exFact) < new Date(appliedFilters.exFactFrom)) matchDates = false;
    if (appliedFilters.exFactTo && item.exFact !== 'N/A' && new Date(item.exFact) > new Date(appliedFilters.exFactTo)) matchDates = false;
    return matchSPO && matchProduct && matchDates && ((appliedFilters.materialStatus || appliedFilters.materialType) ? item.materials.length > 0 : true);
  });

  // --- LOGIKA FILTER TABEL HALAMAN 2 (SCHEDULE) ---
  const filteredDataTab2 = data.filter(item => {
    const matchSPO = item.spo.toLowerCase().includes(appliedSchedFilters.spo.toLowerCase());
    let matchDates = true;
    if (appliedSchedFilters.startFrom && new Date(item.pStart) < new Date(appliedSchedFilters.startFrom)) matchDates = false;
    if (appliedSchedFilters.startTo && new Date(item.pStart) > new Date(appliedSchedFilters.startTo)) matchDates = false;
    if (appliedSchedFilters.exFrom && item.exFact !== 'N/A' && new Date(item.exFact) < new Date(appliedSchedFilters.exFrom)) matchDates = false;
    if (appliedSchedFilters.exTo && item.exFact !== 'N/A' && new Date(item.exFact) > new Date(appliedSchedFilters.exTo)) matchDates = false;
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <section>
          <FileUploader setOutsFile={setOutsFile} setPoFile={setPoFile} setBookFile={setBookFile} onProcess={handleProcessCSV} isProcessing={isProcessing} />
        </section>
        ...
        {data.length > 0 && (
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 bg-slate-200/60 p-1.5 rounded-lg border border-slate-300/40">
            <div className="flex gap-2 w-full sm:w-auto">
              <button onClick={() => setActiveTab('monitoring')} className={`flex items-center justify-center gap-2 px-5 py-2 text-sm font-bold rounded-md transition-all w-full sm:w-auto ${activeTab === 'monitoring' ? 'bg-white text-blue-700 shadow-md' : 'text-slate-600 hover:text-slate-900'}`}>
                <Layers size={16} /> Material Monitoring
              </button>
              <button onClick={() => setActiveTab('schedule')} className={`flex items-center justify-center gap-2 px-5 py-2 text-sm font-bold rounded-md transition-all w-full sm:w-auto ${activeTab === 'schedule' ? 'bg-white text-blue-700 shadow-md' : 'text-slate-600 hover:text-slate-900'}`}>
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-100 font-medium">
                      {filteredDataTab2.map((spo, i) => (
                        <tr key={i} className={`hover:bg-slate-50 ${spo.isDelayRisk ? 'bg-red-50/40 text-red-900' : 'text-slate-800'}`}>
                          <td className="p-3">
                          ...
                          <td className="p-3 whitespace-nowrap">{spo.pStart} <span className="text-[10px] font-bold text-slate-400 block">{spo.pStartWk}</span></td>
                          <td className="p-3 whitespace-nowrap font-semibold">{spo.pFinish} <span className="text-[10px] font-bold text-slate-400 block">{spo.pFinishWk}</span></td>
                          <td className="p-3 whitespace-nowrap font-bold">{spo.exFact} <span className="text-[10px] font-bold text-purple-600 block">{spo.exFactWk}</span></td>
                        </tr>
                      ))}
                      {filteredDataTab2.length === 0 && (
                        <tr><td colSpan="4" className="text-center p-8 text-gray-400 italic">Tidak ada jadwal cocok dengan kriteria filter.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
              ...
              <div className="lg:col-span-6 space-y-4">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 border-l-4 border-l-emerald-500 p-4">
                  ...
                  <div className="max-h-36 overflow-y-auto text-xs space-y-1.5 pr-1">
                    ...
                    {matSummary.gudang.length === 0 && <p className="text-gray-400 italic text-[11px] p-2">Kosong / tidak ada komponen.</p>}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 border-l-4 border-l-cyan-500 p-4">
                  ...
                  <div className="max-h-36 overflow-y-auto text-xs space-y-1.5 pr-1">
                    ...
                    {matSummary.free.length === 0 && <p className="text-gray-400 italic text-[11px] p-2">Kosong / tidak ada komponen.</p>}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 border-l-4 border-l-amber-500 p-4">
                  ...
                  <div className="max-h-36 overflow-y-auto text-xs space-y-1.5 pr-1">
                    ...
                    {matSummary.poReady.length === 0 && <p className="text-gray-400 italic text-[11px] p-2">Kosong / tidak ada komponen.</p>}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 border-l-4 border-l-red-500 p-4">
                  ...
                  <div className="max-h-36 overflow-y-auto text-xs space-y-1.5 pr-1">
                    ...
                    {matSummary.poNeed.length === 0 && <p className="text-emerald-600 font-bold italic text-[11px] p-2">Sempurna! Semua kekurangan sisa produksi sudah di-PO kan.</p>}
                  </div>
                </div>
        ...
        {data.length === 0 && (
          <div className="text-center py-20 text-slate-400 bg-white rounded-lg border border-dashed border-slate-300">
            ...
            <p className="text-sm mt-1">Silakan upload berkas CSV atau sync server data untuk mengaktifkan sistem.</p>
          </div>
        )}
