...
const Dashboard = ({ data }) => {
  ...
  const criticalSPOs = data.filter(
    (item) => item.hasBalance && item.level === 'Critical',
  ).length;
  const warningSPOs = data.filter(
    (item) => item.hasBalance && item.level === 'Warning',
  ).length;
  const lowSPOs = data.filter(
    (item) => item.hasBalance && item.level === 'Low',
  ).length;
  ...
  return (
    <div className="mb-6 space-y-4">
        ...
        <div className="bg-white p-4 rounded-lg shadow-xs border border-gray-200 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xl font-bold text-slate-800">
              {lackingSPOs}{' '}
              <span className="text-xs font-normal text-gray-400">SPO</span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white p-3 rounded-lg shadow-xs border border-gray-200 flex flex-wrap items-center justify-between gap-4 text-xs">
        <div className="text-gray-500 font-bold tracking-wider uppercase text-[10px]">
          {' '}
          Breakdown Tingkat Resiko Outs:{' '}
        </div>
        <div className="flex gap-6">
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-red-500"></span>
            <span className="text-gray-600 font-medium">Critical:</span>
            <span className="font-bold text-red-700 bg-red-50 px-2 py-0.5 rounded-sm">
              {criticalSPOs}
            </span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-yellow-700 bg-yellow-50 px-2 py-0.5 rounded-sm">
              {warningSPOs}
            </span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-green-700 bg-green-50 px-2 py-0.5 rounded-sm">
              {lowSPOs}
            </span>
          </div>
        </div>
