  ...
  const handleGroupClick = (spoId, groupName) => {
    setActiveRowFilters((prev) => {
      ...
      if (currentFilters.includes(groupName)) {
        return {
          ...prev,
          [spoId]: currentFilters.filter((g) => g !== groupName),
        };
      } else {
        ...
        <table className="w-full text-left text-sm table-fixed min-w-[1100px] md:table-auto">
          <thead className="bg-slate-800 text-white">
            <tr>
              ...
              <th className="p-4">
                Material Status, Allocation Details & PO ETA
              </th>
            </tr>
          </thead>
            ...
            {data.map((spo, idx) => {
              ...
              const displayedMaterials =
                selectedGroups.length > 0
                  ? spo.materials.filter((mat) =>
                      selectedGroups.includes(mat.group),
                    )
                  : spo.materials;
              ...
              return (
                <tr
                  key={idx}
                  className={`hover:bg-slate-50 transition-colors ${spo.isDelayRisk ? 'bg-rose-50/10' : ''}`}
                >
                  ...
                  <td className="p-4 align-top">
                    {!spo.hasBalance ? (
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold inline-block text-center w-full ${
                          spo.level === 'Critical'
                  ...
                  <td className="p-4 align-top">
                    ...
                    <div className="text-xs text-slate-600 mt-1 leading-relaxed wrap-break-word">
                      {spo.product}
                    </div>
                  </td>
                  ...
                  <td className="p-4 align-top text-xs space-y-1.5">
                    <div>
                      ...
                      {spo.pStartWk && (
                        <span className="ml-1.5 px-1 bg-slate-100 text-slate-600 rounded-sm text-[10px] font-bold">
                          {spo.pStartWk}
                        </span>
                      )}
                    </div>
                    <div
                      className={
                        spo.isDelayRisk ? 'text-red-600 font-bold' : ''
                      }
                    >
                      <span className="text-gray-400 inline-block w-12 font-normal">
                        Finish:
                      </span>
                    ...
                    <div className="pt-1 mt-1 border-t border-gray-100">
                      ...
                      {spo.exFactWk && (
                        <span className="ml-1.5 px-1 bg-purple-50 text-purple-700 rounded-sm text-[10px] font-bold">
                          {spo.exFactWk}
                        </span>
                      )}
                    </div>
                  ...
                  <td className="p-4 align-top">
                    {spo.materials.length === 0 ? (
                      <div className="text-xs text-emerald-700 font-bold bg-emerald-50/80 p-3 rounded-md text-center border border-emerald-200 flex items-center justify-center gap-1.5">
                        <CheckCircle size={14} /> Material lengkap, siap
                        produksi.
                      </div>
                    ) : (
                      <div className="space-y-3 max-h-80 overflow-y-auto pr-2 border border-slate-100 bg-slate-50/40 p-2 rounded-md shadow-inner">
                        {displayedMaterials.map((mat, i) => {
                          ...
                          if (mat.statusType === 'booked') {
                            ...
                            mat.statusType === 'freestock_kurang_po' ||
                            mat.statusType === 'po'
                          ) {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div
                                className={`p-2 rounded-sm border text-[11px] leading-relaxed font-medium ${statusBgColor}`}
                              >
